...
function cleanup(str) {
  if (!str || typeof str !== "string")
    return str;
  return str.replace(/ data-v-inspector="[^"]*"/g, "").replace(/<!--\[-->/g, "").replace(/<!--]-->/g, "").replace(/<template>/g, "").replace(/<template[^>]*>/g, "").replace(/<\/template>/g, "").replace(/<tailwind-clean>/g, "").replace(/<tailwind-clean[^>]*>/g, "").replace(/<\/tailwind-clean>/g, "");
}
...
function isEndOfTagSection$1(c) {
    return c === CharCodes$1.Slash || c === CharCodes$1.Gt || isWhitespace$3(c);
}
function isNumber(c) {
...
let Tokenizer$1 = class Tokenizer {
    ...
    stateInSpecialTag(c) {
        if (this.sequenceIndex === this.currentSequence.length) {
            if (c === CharCodes$1.Gt || isWhitespace$3(c)) {
    ...
    stateInAttributeValueNoQuotes(c) {
        if (isWhitespace$3(c) || c === CharCodes$1.Gt) {
            this.cbs.onattribdata(this.sectionStart, this.index);
            this.sectionStart = -1;
...
function process (html, metadata, options, picker, findBaseElements, walk) {
  ...
  const document = parseDocument$1(html, { decodeEntities: options.decodeEntities });
...
const EBody = vue.defineComponent({
  name: "EBody",
  setup(_, { slots }) {
    return () => {
      const bodyNode = vue.h("body", { "data-id": "__vue-email-body" }, slots.default?.());
      return bodyNode;
    };
  }
});

const EButton = vue.defineComponent({
  name: "EButton",
  props: {
    px: {
      type: [String, Number]
  ...
  setup(props, { slots }) {
    ...
    return () => {
      return vue.h(
        ...
        [
          vue.h("span", { innerHTML: firstSpan }),
          vue.h(
            "span",
            {
          ...
          vue.h("span", { innerHTML: secondSpan })
        ]
      );
    };
  }
...
const EColumn = vue.defineComponent({
  name: "EColumn",
  setup(_, { slots }) {
    return () => {
      return vue.h(
...
const EContainer = vue.defineComponent({
  name: "EmailContainer",
  setup(_, { slots }) {
    return () => {
      return vue.h(
        ...
        [vue.h("tbody", [vue.h("tr", { style: "width: 100%" }, [vue.h("td", {}, slots.default?.())])])]
      );
    };
  }
});

const EFont = vue.defineComponent({
  name: "EFont",
  props: {
    fontFamily: {
      type: String,
...
const EHead = vue.defineComponent({
  name: "EHead",
  setup(_, { slots }) {
    return () => {
      return vue.h("head", [vue.h("meta", { httpEquiv: "Content-Type", content: "text/html; charset=UTF-8" }), slots.default?.()]);
    };
  }
});

const EHeading = vue.defineComponent({
  name: "EHeading",
  props: {
    as: {
      type: String,
...
const EHtml = vue.defineComponent({
  name: "EHtml",
  props: {
    lang: {
      type: String,
...
const EImg = vue.defineComponent({
  name: "EImg",
  props: {
    src: {
      type: String,
  ...
  setup(props) {
    ...
    const src = vue.ref(props.src);
    if (baseUrl) {
      src.value = ufo.resolveURL(baseUrl, src.value);
      src.value = ufo.cleanDoubleSlashes(src.value);
    }
    return () => {
      return vue.h("img", {
...
const ELink = vue.defineComponent({
  name: "ELink",
  props: {
    href: {
      type: String,
...
const EPreview = vue.defineComponent({
  name: "EPreview",
  setup(_, { slots }) {
    ...
    return () => {
      return vue.h(
        ...
        [text.value, vue.h("div", [renderWhiteSpace(text.value)])]
      );
    };
  }
});

const ERow = vue.defineComponent({
  name: "ERow",
  setup(_, { slots }) {
    return () => {
      return vue.h(
        ...
        [vue.h("tbody", { style: "width: 100%" }, [vue.h("tr", { style: "width: 100%" }, slots.default?.())])]
      );
    };
  }
});

const ESection = vue.defineComponent({
  name: "ESection",
  setup(_, { slots }) {
    return () => {
      return vue.h(
        ...
        [vue.h("tbody", [vue.h("tr", [vue.h("td", slots.default?.())])])]
      );
    };
  }
});
...
const ETailwind = vue.defineComponent({
  name: "ETailwind",
  props: {
    config: {
      type: Object,
  ...
  async setup(props, { slots }) {
    ...
    let headStyles = [];
    const markupWithTailwindClasses = await serverRenderer.renderToString(vue.h("div", $default)).then((html2) => html2.replace(/^<div[^>]*>|<\/div>$/g, ""));
    const markupCSS = tailwind.useRgbNonSpacedSyntax(
      await tailwind.getCssForMarkup(markupWithTailwindClasses, props.config)
    );
    ...
    return () => {
      return vue.h("tailwind-clean", { innerHTML: html });
    };
  }
});

const EText = vue.defineComponent({
  name: "EText",
  setup(_, { slots }) {
    return () => {
      return vue.h(
...
const EMarkdown = vue.defineComponent({
  name: "EMarkdown",
  props: {
    source: {
      type: String,
...
const EStyle = vue.defineComponent({
  name: "EStyle",
  setup(_, { slots }) {
    return () => {
      return vue.h(
...
exports.EImg = EImg;
exports.ELink = ELink;
exports.EMarkdown = EMarkdown;
exports.EPreview = EPreview;
exports.ERow = ERow;
exports.ESection = ESection;
exports.EStyle = EStyle;
exports.ETailwind = ETailwind;
exports.EText = EText;
exports.VueEmailPlugin = VueEmailPlugin;
